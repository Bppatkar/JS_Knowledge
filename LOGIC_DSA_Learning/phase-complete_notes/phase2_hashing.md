...
- **What it wastes:** repeated linear scans for lookups that could, in principle, be
  answered in constant time if you already knew _where_ to look.
...
O(1) average. Recognizing _when this trade is worth making_ is the actual skill —
...
- **Open Addressing:** on a collision, the table probes for the _next_ available slot
...
**Why arrays cannot solve every lookup problem:** arrays give O(1) access _only_ when
...
- **Why keys become strings:** a plain JS object's keys are always coerced to strings
  ...
  (`obj[1]` and `obj["1"]` are the _same_ key).
...
- **Why Map exists:** to fix exactly the problems Object has — keys can be _any_ type
...
- **Primitive vs object keys:** primitive keys (numbers, strings) are compared by value;
  object keys are compared by _reference_ — two different object literals with identical
...
- **Garbage collection concept:** a WeakMap/WeakSet holds its keys/values _weakly_ —
...
- **What it wastes:** if you need frequency information for _many_ values (or all of
...
- **Why waste is avoidable:** a single O(n) pass can populate a frequency map for
  _every_ distinct value at once — after that, any single frequency lookup is O(1).
...
The invariant maintained is: _`map[value]` always equals the exact number of times
`value` has been seen so far in the input_. This holds by direct construction — every
...
The problem statement mentions: _anagram_, _frequency_, _majority element_, _how many
times does X occur_, _k most/least frequent_, or requires comparing the multiset of
...
- ❌ Only checking `map.has(char)` without checking counts reaching exactly zero —
  this misses cases where one string has _more_ of a character than the other.
...
> Mental model in one phrase: _a ballot-counting tally sheet._
> Complexity: _O(n) time, O(k) space._
...
average time, without caring _how many_ times it occurred — only _whether_ it occurred.
...
The invariant is: _at any point during the scan, the Set contains exactly the distinct
values encountered so far_. Since every new element is added to the set the moment it's
...
The problem statement mentions: _contains duplicate_, _does X exist_, _has this been
seen before_, _cycle detection_ (Happy Number — detecting a repeated state), or any
...
- ❌ For cycle-detection style problems (Happy Number), forgetting that the _state_
  being tracked is the computed value at each step, not the original input — the Set
  must track the sequence of _transformed_ values.
...
up to that point. The key implementation detail is checking presence _before_ adding the
...
- **LC128** — Longest Consecutive Sequence (introduces Presence Checking used to find
  _sequence starting points_ — only start counting a run from a value whose predecessor
  is absent from the set, which keeps the whole algorithm O(n) instead of O(n log n))

> **Revision Box**
> Formula/invariant: _check `.has()` before `.add()`, always in that order._
> Mental model in one phrase: _a doorperson's guest checklist._
> Complexity: _O(n) time, O(n) space._

---

## Pattern 3 — Counting Pattern

_(Marked 80/20 compressed in the original training — lighter treatment, still complete
enough to recognize and apply.)_
...
_generalized_ — instead of just building the map and reading it once at the end, the
count is often accumulated incrementally _during_ the same pass that builds the map,
...
The problem statement asks _"count the number of..."_ pairs, subarrays, unique values,
...
- ❌ Updating the map _before_ using it to accumulate the count for the current
...
> **Revision Box**
> Formula/invariant: _look up first (accumulate count), then update the map — order matters._
> Mental model in one phrase: _ask "how many valid partners exist so far," then join the party yourself._
> Complexity: _O(n) time, O(n) space._
...
- **What it wastes:** if two items belong to the same group, that fact can usually be
  detected _independently_, per item, by computing a canonical signature — there's no
...
The problem statement mentions: _group by_, _cluster_, _anagram grouping_, or any
...
> Mental model in one phrase: _sorting mail into bins by zip code._
> Complexity: _O(n · k log k) time (sort-based signature), O(n · k) space._
...
- **Why waste is avoidable:** record the useful information _at the moment it's first
  known_ — while iterating — into a map, so any later lookup is O(1).
...
The invariant is: _`map[key]` always holds the most recent (or first, depending on the
problem) piece of information associated with `key`, as of the current point in the
scan_. Because the map is updated at the exact moment the relevant information becomes
...
The problem statement asks to find _a pair, a complement, an index of a previously seen
value_, or requires transforming one value into another via a known relationship (e.g.,
...
- Duplicate values in Two Sum where `target = 2 * value` — correctly handled by checking
  the map _before_ inserting the current element, since the _previous_ occurrence's
...
_complement_ of the current number has already been seen, which turns an O(n²) pair
...
> **Revision Box**
> Formula/invariant: _check the map for what you need BEFORE inserting the current element._
> Mental model in one phrase: _a librarian's card catalog — title maps straight to shelf number._
> Complexity: _O(n) time, O(n) space._
...
_(End of Part 1 — Sections 1–2 (Fundamentals, JS Hash Structures), and Patterns 1–5.
Continue with Part 2: Patterns 6–10 + Back Matter.)_

# Phase2_Hashing.md — Part 2

### Patterns 6–10 + Back Matter

_(Continues directly from Part 1 — Sections 1–2, Patterns 1–5. Concatenate after Part 1.
No front matter repeated here.)_
...
being able to answer range-sum _queries_ in O(1) (which requires the array to be static
...
- **What it wastes:** for every `right`, brute force re-derives the sum of every
  ...
  _specific_ prefix value (`currentPrefix - k`) has occurred before.
...
The problem statement mentions: _subarray sum equals k_, _continuous subarray with a
sum divisible by k_, _zero-sum subarray_, or any "does a contiguous run summing to X
...
> Mental model in one phrase: _a notebook of every past balance — ask "did I ever have exactly this much less?"_
> Complexity: _O(n) time, O(n) space. `map.set(0, -1)` for indices, `map.set(0, 1)` for counts._

---

## Pattern 7 — Hash + Sliding Window

_(Marked 80/20 in the original training — Sliding Window mechanics were already
mastered in Phase 1; this section focuses on what's genuinely **new**: using a
HashMap/HashSet as the window's state instead of a scalar.)_
...
`while` invalid — the only difference is _what_ is being maintained as state.
...
- **Requirement decomposition:** for problems like Minimum Window Substring or
  Permutation in String, the window must satisfy a _multi-part_ requirement (frequency
  of _every_ required character simultaneously) — this requires tracking not just a
...
The problem statement needs a _contiguous_ window (Sliding Window signal from Phase 1)
...
> **Revision Box**
> Formula/invariant: _Phase 1 Sliding Window skeleton + Phase 2 Hash state (map or set) = this pattern._
> Mental model in one phrase: _the same camera frame from Phase 1, now with a tally board attached._
> Complexity: _O(n) time, O(k) space._
...
_(Marked 80/20 compressed in the original training — lighter treatment.)_
...
running XOR, a running parity), and the HashMap gives fast O(1) lookup of _where or how
often_ that cumulative state has occurred before — enabling both counting questions
...
the earliest index, are both common and easy mistakes — the _mechanism_ (prefix +
hash) is identical either way, but the _payload_ is not.
...
function maxSubArrayLen(nums, k) {
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    if (!earliestIndexOf.has(prefix)) {
      // only set the FIRST time this prefix occurs
...
The problem statement mentions _"maximum length subarray"_ combined with a sum/prefix
...
- ❌ Overwriting `earliestIndexOf[prefix]` on every occurrence instead of only the first
  — this would use a _later_ (shorter) starting point instead of the earliest possible
...
> **Revision Box**
> Formula/invariant: _longest → store earliest index (write-once); counting → store frequency (always increment)._
> Mental model in one phrase: _same notebook of past balances, but now you only ever write down the FIRST time you hit each balance._
> Complexity: _O(n) time, O(n) space._
...
Hash + Two Pointer is the recognition that some problems are better solved by _sorting_
...
- **What it wastes:** for a _fixed_ first element, finding a pair summing to a specific
...
- **Why waste is avoidable:** fix one element at a time (O(n) choices), and for each
  fixed element, use Opposite Direction Two Pointer on the _remaining sorted subarray_
...
- **This pattern:** recognize when sorting first and reusing Phase 1's two-pointer
  machinery is a _better_ fit than reaching for a HashMap, particularly when duplicate
  _triplets_ need to be avoided in the output, since sorted order makes duplicate-skipping
...
The problem statement mentions: _3Sum, 4Sum_, or any "find k numbers that sum to a
...
> **Revision Box**
> Formula/invariant: _sort → fix i → two pointers → compare sum → skip duplicates._
> Mental model in one phrase: _splitting a bill three ways using a sorted line and two people walking inward._
> Complexity: _O(n²) time, O(1) auxiliary space (excluding output)._
...
- **What it wastes:** relying purely on a built-in `Map`/`Set` without understanding
  _why_ it's fast means you can't reason about worst-case behavior, can't design a
...
The problem statement explicitly says _"design a HashSet/HashMap"_ without using the
...
class MyHashSet {
  ...
  add(key) {
    ...
    if (!bucket.includes(key)) {
      // avoid duplicate entries within the same bucket
      bucket.push(key);
    }
  }
...
- ❌ For composite keys, using a delimiter that could itself appear inside the
  individual values (e.g., joining with `,` when a value can _contain_ a `,`) — this can
...
> Mental model in one phrase: _an apartment building's mailboxes — one box can hold mail for more than one resident._
> Complexity: _O(1) average, O(k) worst case per operation; O(numBuckets + n) space._
...
- **Map value must match the requirement.** The single most common design mistake across
  ...
  count when an index was needed. Always ask "what does the _question_ actually need me
...
- **Longest prefix range needs earliest index.** For "longest subarray satisfying X"
  problems, only write a prefix value into the map the _first_ time it's seen — a later
...
## Phase 3 Preview — What Hashing Does _Not_ Cover
...
- **Stacks & Monotonic Stack** — problems where the _relative order_ of unprocessed
...
_(End of Part 2 — Patterns 6–10, Back Matter. This completes Phase2_Hashing.md.)_
